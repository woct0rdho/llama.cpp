...
// Route-bounded ids layout for the 512-expert, top-10 MoE at large token counts: the per-expert
// tile index has to stay inside 16 bits, which the generic helper cannot guarantee there.
...
bool ggml_cuda_launch_mm_ids_bounded(
        ...
        int channels, int ids_stride, int token_stride, bool inverse) {
    ...
    ggml_cuda_pool_alloc<uint32_t> sorted(ctx.pool(), size_t(chunks)*MM_IDS_ROUTE_TILE);
    ggml_cuda_pool_alloc<int32_t> offsets(ctx.pool(), size_t(chunks)*experts);
    ggml_cuda_pool_alloc<int32_t> starts(ctx.pool(), size_t(chunks)*experts);
