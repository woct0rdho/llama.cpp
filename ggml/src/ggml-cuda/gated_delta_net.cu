...
// Non-KDA variant with the per-token inputs (q, k, g, beta, v) of a token tile staged in shared memory, and
// COLS state columns per warp. k/q are then read once per token for COLS columns instead of once per column,
// and COLS times fewer waves compete for issue slots - the untiled kernel spends ~50 VALU per token per wave
...
gated_delta_net_tiled_cuda(const float * q,
    ...
    const int      colw     = threadIdx.y * COLS;      // first column of this warp within the block
...
static void launch_gated_delta_net(
    ...
    // the tiled kernel only pays off once a tile is full, and has so far only been tuned on RDNA3.5
    static const bool no_tiled = getenv("GGML_CUDA_NO_GDN_TILED") != nullptr;
